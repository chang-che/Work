...
for (i in 1:10) 
  ...
  for (j in 1:length(listofn)) {
    ...
    listofplot[[j]] = getplot(main = paste(listofn[j], 'fitted vs true 5fold-cv by remMap----',i), 
                              xlab = paste0(listofn[j], ' Predictted') ,ylab = paste0(j, ' True'), Y.fitted1[[j]], Y.testData[[j]]) 
  }
...
pdf(file = "/export/home/chche/WTC_Project/Results/PCL_H&A&R&N_variables_within_response_v2.pdf")
for (j in listofn){
  cor_remMap = c()
