...
for (i in 1:10) 
{
  ...
  for (j in 1:length(listofn)) {
    ...
    listofplot[[j]] = getplot(main = paste(listofn[j], 'fitted vs true 10*2 by remMap----',i), 
  ...
  for (j in 1:length(listofn)) {
    ...
    listofplot[[j]] <- getplot(main = paste(listofn[j], 'fitted vs true 10*2 by ElasticNet--',i), 
...
pdf(file = "/export/home/chche/WTC_Project/Results/PCL&PHQ9&LRS_total_variables_within_response_10*2_v2.pdf")
textplot(paste0('Plots and summary of 10 tests.', '\n',
                'Genes with the low counts are deleted', '\n',
                'Data is randomly splitted into test set and train set with ratio of 3:7'), halign = 'left', valign = 'top', cex = 0.7)
...
for (j in 1:length(listofn)){
  ...
  par(mfrow = c(1,1))
  textplot(paste0("Notice that only-intercept model is discarded,\n","because in that case intersection of all 10 models will be empty.", '\n',
                  "Average number of non-zero coefficients in the remMap model for response ", listofn[j], ' is ',mean(num_remMap),'\n',
  ...
  textplot(paste0("Notice that only-intercept model is discarded,\n","because in that case intersection of all 10 models will be empty.", '\n',
                  "Average number of nonzero coefficients in the Elastic net model for response ", listofn[j], ' is ', mean(sapply(beta_Elastic, length)),'\n',
