...
colnames(Y.m2) = listofn
Y.m2 = Y.m2[ind,]
Y.m2 = scale(Y.m2, center = apply(Y.m2, 2, mean), scale = apply(Y.m2, 2, sd))
Y.m2 = as.data.frame(Y.m2)

sam_ind = sample(nrow(X.m2))
X.m2 <- X.m2[sam_ind,]
Y.m2 <- Y.m2[sam_ind,]


#Create 5 equally size folds
folds <- cut(seq(1,nrow(Y.m2)),breaks=5,labels=FALSE)
#Perform 5 fold cross validation
for (i in 1:5) {
  #Segement your data by fold using the which() function 
  testIndexes <- which(folds==i,arr.ind=TRUE)
  X.testData <- X.m2[testIndexes, ]
  Y.testData <- Y.m2[testIndexes, ]
  X.trainData <- X.m2[-testIndexes, ]
  Y.trainData <- Y.m2[-testIndexes, ]
...
pdf(file = "/export/home/chche/WTC_Project/Results/PCL_H&A&R&N_remMap_vs_ElasticNet.pdf")
for (j in listofn){
