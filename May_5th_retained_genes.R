...
respond <- c('MDD', 'PHQ9')
batch_num <- c(3,4)
pdf(file = "/export/home/chche/WTC_Project/Results/8_tests_2_genes_retained_10times.pdf")
      ...
      for (glist_ind in 1:1){ # 1:Top 2000 genes by 2-step filtering   2: 40 candidate genes by 2-step filtering.
        ...
        for (batch_ind in 1:1){
          ...
          for (j in 2:2){ # MDD or PHQ9 as the response variable
            ...
            g_r <- names(sort(abs(coef(opt_model)[,1]), decreasing = T)[2:51])
            
            genes_retained_2[[PHQ_ind]] <- list(g_r)
            fit = lm(Y.m~X.m[, g_r])
            cvElaNet_pred_WTC = X.testData[, g_r]%*%fit$coefficients[-1] + fit$coefficients[1]
            
            cvElaNet_pred_Mostafari = Mostafavi.Xm[, g_r]%*%fit$coefficients[-1] + fit$coefficients[1]
        ...
        for(p in 1:1){ #batch number
          for(q in 2:2){ #response number
            par(mfrow = c(1,1))
            textplot(paste('Train on', respond[q], 'of batch', batch_num[p], '\n',
                           ...
                           'AUC are', paste(auc_list[[p]][[q]], collapse = ' '), 'respectively','\n'), 
                     halign = 'left', valign = 'top', cex = 1.0)
            
            par(mfrow = c(1,1))
            multiplot(plotlist = plot_list[[p]][[q]], cols = 2)
            textplot(matrix(unlist(genes_retained_2[[PHQ_ind]]), ncol = 5), halign = 'left', valign = 'top', cex = 0.7)
          }
        }
...
dev.off()
save(genes_retained_2, file = '/export/home/chche/WTC_Project/Data/genes_rep_10.Rdata')
