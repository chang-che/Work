...
dds.log <- log2(dds+1)
dds.log <- dds.log[,] # only 274 male samples 
...
respond <- c('MDD','PHQ9')
batch_num <- c(3, 4)
pdf(file = "/export/home/chche/WTC_Project/Results/median_mad_2step_filtered_log(PHQ9)_both.pdf")
textplot(paste('PHQ9 MDD prediction with log transformation \n','and scale by samples :(x-median(x))/mad(x)','\n',
               'log transformed phq9','\n',
               'Use 2000 top genes.','\n',
               'Both gender are tested for Mostafavi.'))
