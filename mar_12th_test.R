...
############################################################
#############################################################
################# the model of elastic net


## re-initialize the listofplot and listofbeta
listofplot <-  list()
listofbeta <-  list()
listofalphas <- list()
listofbetas <- list()
...
alpha_cand = seq(0, 1, 0.01)## candidate alpha values, step = 0.01

Elastic.cv = function(trainID){
  trainX_data = as.matrix(X.m1[trainID,])
  for (j in 1:length(listofn)) {
    trainY_data = as.matrix(Y.m1[trainID,][,j])
  ...
  return(list(plots = listofplot, betas = listofbeta))
  rm(opt_model)
}
  
  results_Elastic <- mclapply(test_list, Elastic.cv, mc.cores = 12)


###############################################
#the summary of the analysis for the comparison
pdf(file = "/export/home/chche/WTC_Project/Results/test_10*3:7_v6.pdf")
...
for (j in 1:length(listofn)){
  ...
  textplot(paste0("Average number of nonzero coefficients in the remMap model for response ", listofn[j], ' is ',mean(num_remMap),'\n',
                  ...
                  "The range of lambda1 and lambda2 are",lamL1.string, lamL2.string,'\n',
                  "The optimal lambdas are",'\n',
                  paste(listoflambdas)), 
  ...
  for (i in 1:nloop) {
    #store the jth response variable's plot into a list for all 10 tests
    plots_Elastic[[i]] = results_Elastic[[i]][['plots']][[j]]
    if (!is.null(unlist(results_Elastic[[i]][['betas']][[j]]))) { #discard the only intercept model
      beta_Elastic <- c(beta_Elastic, results_Elastic[[i]][['betas']][[j]])
