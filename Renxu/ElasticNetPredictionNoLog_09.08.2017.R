...
load(file='/export/home/pfkuan/WTCproject/Epigenetics/Results/Results_Feb2017_RNASeq/plots_06Sept2017/cv.fit_DemoCellAdj_07Sept2017_noLog.RData')
...
train195 =  t(exoncounts_normalized[genes278, trainID])
...
pdf(file = "/export/home/xurren/WTCProject/Results/boxplot_for_riskscores_noLog_09.08.2017.pdf")
theme_update(plot.title = element_text(hjust = 0.5))
ggplot(result_train, aes(x = PTSD, y = riskscore)) + geom_boxplot(aes(color = PTSD_train)) + ggtitle("No Log Exon Counts: Risk score for 195 train samples in old batch")


## predict on 135 test data from batch 3
test135 = t(exoncounts_normalized[genes278, testID2]) 
...
ggplot(result_test135, aes(x = PTSD, y = riskscore)) + geom_boxplot(aes(color = PTSD_test135)) + ggtitle("No Log Exon Counts: Risk score for 135 test samples in old batch")


## predict on 203 test data from batch 4
test203 = t(exoncounts_normalized[genes278, testID1]) 
...
ggplot(result_test203, aes(x = PTSD, y = riskscore)) + geom_boxplot(aes(color = PTSD_test203)) + ggtitle("No Log Exon Counts: Risk score for 203 test samples in new batch")
