...
pdf(file = "/export/home/chche/WTC_Project/Results/PCL&PHQ9&LRS_total_preID.pdf")
textplot(paste0('Plots and summary of 1 test.', '\n',
                'Data is splitted into test set and train set based on pre-specified ID'), halign = 'left', valign = 'top', cex = 0.7)
...
for (j in 1:length(listofn)){
  ...
  for (i in 1:nloop){
    ...
    if (!is.null(listofbeta_remMap[[i]][[j]])) { #discard the only intercept model
  ...
  for (i in 1:nloop) {
    ...
    if (!is.null(listofbeta_Elastic[[i]][[j]])) { #discard the only intercept model
  ...
  num_remMap_Elastic = length(intersect(unlist(listofbeta_Elastic[[i]][[j]]), unlist(listofbeta_remMap[[i]][[j]])))
  
  App_net_10 = Reduce(intersect, all_Elastic)
  textplot(paste0("Number of nonzero coefficients in the remMap model for response ", listofn[j], ' is ',mean(num_remMap),'\n',
                  "Number of nonzero coefficients in the Elastic Net model for response ", listofn[j], ' is ',mean(sapply(beta_Elastic, length)),'\n',
                  100*mean(num_remMap_Elastic/num_remMap), "% of nonzero terms in remMap model is in Elastic Net model" ), halign = 'left', valign = 'top', cex = 0.6)
...
###############################################
#the summary of the analysis for the comparison
pdf(file = "/export/home/chche/WTC_Project/Results/PCL_H&A&R&N_preID.pdf")
textplot(paste0('Plots and summary of 1 test.', '\n',
                'Data is splitted into test set and train set based on pre-specified ID'), halign = 'left', valign = 'top', cex = 0.7)
